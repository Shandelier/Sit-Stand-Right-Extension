...
class ActiveTabTracker {
  ...
  getTabStatus(tab) {
    ...
    return this._tracker[index].blur ? this._tracker[index].blur : false
  }
...
// Hooks for tab change event and managing blur effect
{
  ...
  function updateEffect(isSlouching, tab) {
    ...
    if (isSlouching !== tabEffectState) {
      if (isSlouching) {
        chrome.tabs.insertCSS(null, { file: 'src/css/blur.css' })
        tabTracker.setTabStatus(tabIndex, true)
        return
      }
      chrome.tabs.insertCSS(null, { file: 'src/css/normal.css' })
